...
namespace environs
{
	...
	INLINEFUNC SOCKETSYNC CloseReplaceThreadSocket ( SOCKETSYNC * psock, int replace )
	{
		...
		if ( sock != -1 )
		{
			CSocketLogArg ( "CloseReplaceThreadSocket: Closing [ %i ]", sock );
	...
	void ClearStuntSockets ( ThreadInstance * inst )
	{
		...
		if ( MutexLockA ( inst->stuntSocketLock, "ClearStuntSockets" ) )
		{
			...
			MutexUnlockA ( inst->stuntSocketLock, "ClearStuntSockets" );
		}
    }
	...
	void ReplaceStuntSocket ( ThreadInstance * inst, int replace )
	{
		...
		if ( sock != -1 )
		{
			...
			if ( MutexLockA ( inst->stuntSocketLock, "ReplaceStuntSocket" ) )
			{
				...
				MutexUnlockA ( inst->stuntSocketLock, "ReplaceStuntSocket" );
			}
	...
	bool CloseThreadSocket ( SOCKETSYNC * psock )
	{
		...
		if ( sock != -1 ) {
			CSocketLogArg ( "CloseThreadSocket: Closing [ %i ]", sock );
	...
	ThreadInstance::~ThreadInstance ()
	{
		...
		try
     {
     CloseThreadSocket ( &socketToClose );
		...
		if ( sock != -1 )
		{
			...
			try
			{
				CSocketLogArg ( "ThreadInstance: Destruct closing [ %i ]", sock );
		...
		if ( allocated )
		{
			...
			CVerbVerbArg ( "ThreadInstance [ %s : %i ]: Disposing locks.", ips, port );
			MutexDispose ( &lock );
			MutexDispose ( &spareSocketLock );
		}
	}
	*/
	...
	void MediatorThreadInstance::Dispose ()
	{
		...
		if ( sock != -1 ) {
			socketUdp = -1;
			CSocketLogArg ( "Dispose: Closing [ %i ]", sock );
	...
	MediatorDaemon::~MediatorDaemon ()
	{
		...
		if ( allocated ) {
			MutexDisposeA ( usersDBLock );
			MutexDisposeA ( bannedIPsLock );
			MutexDisposeA ( thread_lock );
			CondDisposeA ( thread_condition );
			MutexDisposeA ( notifyLock );
			MutexDisposeA ( notifyTargetsLock );
			MutexDisposeA ( logMutex );
		}
	}
	...
	void * MediatorDaemon::Acceptor ( void * arg )
	{
		...
		NextClient:
			if ( sock != -1 ) {
				CSocketLogArg ( "Acceptor: Closing [ %i ]", sock );
	...
	void * MediatorDaemon::ClientThreadStarter ( void *arg )
	{
		...
		if ( daemon->isRunning )
		{
            ...
            {
                if ( MutexLockA ( client->stuntSocketLock, "Client" ) )
                {
                    client->CloseStuntSockets ();
                    
                    MutexUnlockA ( client->stuntSocketLock, "Client" );
                }
            }
	...
	*/
	...
	void * MediatorDaemon::ClientThread ( void * arg )
	{
		...
		while ( isRunning && client->socket != -1 ) {
	...
	{
		if ( sourceClient->version < '5' )
			return HandleSTUNTRequestV4 ( sourceClient, ( STUNTReqPacketV4 * ) req );
	...
	bool MediatorDaemon::HandleSTUNTRequestV4 ( ThreadInstance * sourceClient, STUNTReqPacketV4 * req )
	{
		sp ( DeviceInstanceNode ) sourceDeviceSP = sourceClient->deviceSP;
		if ( !sourceDeviceSP )
			return false;
    ...
    int MediatorDaemon::SendBuffer ( SendContext * ctx )
    {
        CVerbVerbArg ( "SendBuffer [ %i ]", ctx->client->socket );
        
        int             rc          = -1,
                        sock;
        char *          cipher      = ctx->sendBuffer;
        unsigned int    sendSize    = ctx->sendSize;
        int             deviceID    = ctx->client->deviceID;
        
        CVerbArgID ( "SendBuffer [ %i ]", ctx->client->socket );
        
        if ( ctx->client->socket == -1 )
            return -1;
        
        if ( !cipher ) {
            if ( ctx->client->encrypt ) {
                sendSize = ctx->msgLen;
                
                if ( !AESEncrypt ( &ctx->client->aes, ( char * ) ctx->msg, &sendSize, &cipher ) || !cipher ) {
                    CErrArgID ( "SendBuffer [ %i ]: Failed to encrypt AES message.", ctx->client->socket );
                    return rc;
                }
            }
            else {
            ...
            ctx->sendSize   = sendSize;
        }
        
        CVerbArgID ( "SendBuffer [ %i ]: [ %i ] bytes", ctx->client->socket, sendSize );
        
        sock = ( int ) ctx->client->socket;
        
        if ( sock != -1 ) {
            rc = ( int ) send ( sock, ( char * ) cipher, sendSize, MSG_NOSIGNAL );
            
            if ( rc == ( int ) sendSize )
                ctx->client->sendFails = 0;
            else
                ctx->client->sendFails++;
        }
        else {
            ctx->client->sendFails++;
        }
        
        CVerbArgID ( "SendBuffer [ %i ]: [ %i ] bytes. Done.", ctx->client->socket, sendSize );
				...
				if ( clientSP ) {
					...
					if ( client->sendFails > 20 || ( checkLast - client->aliveLast ) > ( maxTimeout + compensate ) )
					{
						if ( client->socket != -1 )
						{
							...
							if ( client->stuntSocketsFront != client->stuntSocketsLast )
							{
								if ( MutexLockA ( client->stuntSocketLock, "Watchdog" ) )
								{
									client->CloseStuntSockets ();

									MutexUnlockA ( client->stuntSocketLock, "Watchdog" );
								}
							}
						}
						else {
